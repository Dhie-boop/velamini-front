...
export async function POST(req: Request) {
  try {
    ...
    if (!body || !body.message || !body.virtualSelfId) {
      warn("/api/chat/shared", "Bad request — missing message or virtualSelfId", { body });
    ...
    log("/api/chat/shared", "Incoming chat", { virtualSelfId, visitorName, msgLen: message?.length });
    const apiKey = process.env.DEEPSEEK_API_KEY;

    if (!apiKey) {
      logError("/api/chat/shared", "Missing DEEPSEEK_API_KEY");
    ...
    if (!knowledgeBase) {
      warn("/api/chat/shared", "Virtual self not found in DB", { virtualSelfId });
    ...
    if (!response.ok) {
      ...
      logError("/api/chat/shared", "DeepSeek API error", { status: response.status, errorData });
    ...
    log("/api/chat/shared", "AI response ok", { virtualSelfId, replyLen: finalContent?.length });
    ...
    if (process.env.DATABASE_URL && finalContent) {
      ...
      ]).catch((dbErr) => logError("/api/chat/shared", "Persistence error", { err: String(dbErr) }));
    }

    return NextResponse.json({ text: finalContent, isAnsweringFromKnowledge });

  } catch (error: unknown) {
    logError("/api/chat/shared", "Unhandled exception", { err: error instanceof Error ? error.message : String(error) });
